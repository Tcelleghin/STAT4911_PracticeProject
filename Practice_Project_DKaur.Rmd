...
pcr_cv_rmsep <- RMSEP(pcr.fit, estimate = "CV")$val[1,1,]
pcr_ncomp <- which.min(pcr_cv_rmsep) - 1
pcr_ncomp

print("PCR Loadings")

pcr_loadings <- loadings(pcr.fit)

print("Component 1 (PCR)")
head(sort(pcr_loadings[,1], decreasing = TRUE), 5)
head(sort(pcr_loadings[,1], decreasing = FALSE), 5)

print("Component 2 (PCR)")
head(sort(pcr_loadings[,2], decreasing = TRUE), 5)
head(sort(pcr_loadings[,2], decreasing = FALSE), 5)


pcr.pred <- predict(pcr.fit, test_dat, ncomp = pcr_ncomp)
pcr_mse <- mean((pcr.pred - test_dat$dep_delay)^2)
pcr_mse
```
Examination of the PCA loadings shows that the first principal component is dominated by scheduling and timing variables such as departure and arrival times, indicating that this component captures time-of-day structure in the data. The second principal component is primarily driven by weather-related variables including temperature, humidity, and dew point, representing overall weather conditions. Because these components are constructed to explain predictor variance rather than response variance, they explain relatively little variation in departure delay, which only appears in later components.
...
pls_cv_rmsep <- RMSEP(pls.fit, estimate = "CV")$val[1,1,]
pls_ncomp <- which.min(pls_cv_rmsep) - 1
pls_ncomp

print("PLS Loadings")
pls_loadings <- loadings(pls.fit)

# Top variables in first two PLS components (with sign)

print("Component 1 (PLS)")
head(sort(pls_loadings[,1], decreasing = TRUE), 5)
head(sort(pls_loadings[,1], decreasing = FALSE), 5)

print("Component 2 (PLS)")
head(sort(pls_loadings[,2], decreasing = TRUE), 5)
head(sort(pls_loadings[,2], decreasing = FALSE), 5)


pls.pred <- predict(pls.fit, test_dat, ncomp = pls_ncomp)
pls_mse <- mean((pls.pred - test_dat$dep_delay)^2)
pls_mse

```
Examination of the PLS loadings shows that the first component is dominated by variables directly related to operational delays and scheduling, particularly arrival delay, departure time, and scheduled departure and arrival times. Because PLS constructs components to maximize covariance with the response, this first component explains a substantial proportion of the variation in departure delay and captures the primary mechanism through which delays propagate across flights. Subsequent PLS components continue to emphasize timing-related variables with different relative weights, reflecting secondary scheduling patterns once the main delay effect has been accounted for. This ordering of components explains why PLS achieves strong predictive performance with relatively few components compared to PCA.
